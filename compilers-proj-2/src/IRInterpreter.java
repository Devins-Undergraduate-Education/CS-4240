...
public class IRInterpreter {
    ...
    public void run() throws IRException {
        ...
        IRInstruction entryCall = new IRInstruction(IRInstruction.OpCode.CALL, new IROperand[] { mainFunctionOperand },
                -1);
    ...
    private void executeInstruction(IRInstruction instruction) throws IRException {
        ...
        switch (instruction.opCode) {
            ...
            case BRGEQ: {
                ...
                boolean res = (Boolean) binaryOperation(instruction.opCode, type, a, b);
                if (res)
                    pc.setNextIdx(targetIdx);
                break;
            }
    ...
    private void throwRuntimeException(IRInstruction instruction, String message) throws IRException {
        ...
        while (sit.hasPrevious()) {
            ...
            if (sf.caller == null)
                break;
            System.err.println(
                    "\t" + sf.caller.name + ":" + sf.caller.instructions.get(sf.returnInstIdx - 1).irLineNumber);
        }
