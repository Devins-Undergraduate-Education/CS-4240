...
public class IRReader {
    ...
    /**
     * A text scanner that returns a non-empty line with its line number in each
     * iteration
     */
    ...
    public IRReader() {
        ...
        intrinsics.put("puti", new IRFunction("puti", null,
                Arrays.asList(new IRVariableOperand(IRIntType.get(), "i", null)), null, null));
        intrinsics.put("putf", new IRFunction("putf", null,
                Arrays.asList(new IRVariableOperand(IRFloatType.get(), "f", null)), null, null));
        intrinsics.put("putc", new IRFunction("putc", null,
                Arrays.asList(new IRVariableOperand(IRIntType.get(), "c", null)), null, null));
    }
        ...
        for (IRFunction f : functions)
            for (IRInstruction inst : f.instructions) {
                ...
                if (inst.opCode == IRInstruction.OpCode.CALL) {
                    ...
                    if (callee == null || callee.parameters.size() != inst.operands.length - 1
                    ...
                    if (callee == null || callee.parameters.size() != inst.operands.length - 2
                            || retVar.type != callee.returnType)
                        throw new IRException("Invalid function call", inst.irLineNumber);
                    argIdx = 2;
                } else
                    continue;
    ...
    private IRFunction parseFunction(List<IRLine> irLines)
        ...
        for (int i = 2; i < signatureTokens.length; i += 2) {
            ...
            if (!paramName.matches("^[A-Za-z_][A-Za-z0-9_]*$"))
                throw new IRException(String.format("Invalid parameter name '%s'", paramName),
        ...
        List<IRVariableOperand> localVars = new ArrayList<>();
        for (IRVariableOperand var : variableMap.values()) {
            if (!params.contains(var)) {
                localVars.add(var);
            }
        }

        return new IRFunction(functionName, retType, params, localVars, instructions);
    }
        ...
        for (String varName : varNames) {
            ...
            if (m.matches()) { // Array
                ...
                if (!varNamePattern.matcher(arrayName).matches())
                    throw new IRException(String.format("Invalid variable name '%s'", arrayName),
    ...
    private IRInstruction parseInstruction(IRLine irLine,
        ...
        switch (opCode) {
            ...
            case CALL: {
                ...
                getConstantOrVariableOperands(instruction, operands, tokens, 2, tokens.length - 1, variableMap,
                        lineNumber);
                break;
            }
            case CALLR: {
                ...
                getConstantOrVariableOperands(instruction, operands, tokens, 3, tokens.length - 1, variableMap,
                        lineNumber);
                IRType t0 = getDataType(operands.get(0));
